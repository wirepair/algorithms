...
/*
func mergeSort(data Sorter, aux []interface{}, lo, hi int) {
	if hi <= lo {
		return
	}
	mid := lo + (hi-lo)/2
	...
	inPlaceMerge(data, aux, lo, mid, hi)
}

func inPlaceMerge(data Sorter, aux []interface{}, lo, mid, hi int) {
	switch t := data.(type) {
	case StringSlice:
		for k := lo; k <= hi; k++ {
			aux[k] = t[k]
		...
		for k := lo; k <= hi; k++ {
			if i > mid {
				j += 1
				t[k] = aux[j].(string) // this copying is unnecessary
			} else if j > hi {
				i += 1
				t[k] = aux[i].(string)
			} else if t.Less(aux[j].(int), aux[i].(int)) {
				j += 1
				t[k] = aux[j].(string)
			} else {
				i += 1
...
func InPlaceMergeSort(data Sorter) {
	fmt.Printf("Using inplacemerge sort\n")
